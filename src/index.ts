...
async function getDirs (): Promise<string> {
  return await new Promise((resolve, reject) => {
    ...
    getDirectories(path).forEach((dir) => {
...
async function deleteFile (filepath: PhotoPath): Promise<void> {
  return await Utils.deleteFile(filepath).catch(err => {
...
async function fixExifDate (prefix: string, filepath: PhotoPath, dir: DirInfos, dateFixStat: Stat, exiftool: ExifTool.ExifTool): Promise<string> {
  return await new Promise((resolve, reject) => {
    ...
    exiftool.read(filepath)
      ...
      .then(async (originalDate) => {
        const newDate = new Date(originalDate === null ? '' : originalDate)
...
async function createCopy (filepath: PhotoPath, finalPhotoPath: PhotoPath): Promise<boolean> {
  return await Utils.copyFile(filepath, finalPhotoPath, true)
    .then(() => {
...
async function checkPhotos (photos: PhotoSet, dir: DirInfos, exiftool: ExifTool.ExifTool): Promise<string> {
  ...
  if (count < 1) {
    return await Promise.resolve('found no photos in dir "' + dir.name + '"')
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    await compress(prefix, photo, 'smallfry')
      ...
      .then(async () => await fixExifDate(prefix, photo, dir, Stats.dateFix1, exiftool))
      .then(() => {
        Stats.exifRepair2.skip++
        Stats.dateFix2.skip++
      })
      .catch(message => {
        if (typeof message.includes !== 'function' || message.includes('failed at writing date exif') === true) {
          ...
          return repairExif(prefix, photo, Stats.exifRepair2)
            .then(async () => await fixExifDate(prefix, photo, dir, Stats.dateFix2, exiftool))
...
async function checkNextDir (exiftool: ExifTool.ExifTool): Promise<string> {
  ...
  const dir = firstDir === undefined ? '' : firstDir
  ...
  try {
    ...
    return await checkNextDir(exiftool)
  } catch (err) {
    Logger.error(err)
    return err.message
  }
...
export async function startProcess (): Promise<void> {
  ...
  Logger.start(app)
  Stats.start()
  return await getDirs()
    .then(async () => await checkNextDir(exiftool))
